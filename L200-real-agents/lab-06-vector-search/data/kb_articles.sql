...
-- Idempotent DDL: CREATE IF NOT EXISTS + MERGE by primary key `article_id`.
...
-- invalidated. Rerunning also does NOT duplicate rows.
...
USING (
  ...
  ) AS v (article_id, title, topic, body, published_at)
) s
...
WHEN NOT MATCHED THEN INSERT (
  article_id, title, topic, body, published_at
) VALUES (
  s.article_id, s.title, s.topic, s.body, s.published_at
);
