...
- A `hardware-store-ms-<your-username>` Model Serving endpoint hosting the same agent.
...
| File | Purpose |
|---|---|
| `greeter.yaml` | The agent config -- one LLM, one agent, no tools. Deploys to Databricks Apps. |
| `greeter_model_serving.yaml` | Same greeter, deploys to Model Serving instead. Adds a `registered_model:` block. |
| `notebook.py` | Walk through the YAML, build the agent, test, deploy to Apps (Steps 1-5), then to Model Serving (Step 6). |
...
[Lab 2](../lab-02-uc-tools/) -- give the agent real tools (Unity Catalog SQL functions over a products table).
