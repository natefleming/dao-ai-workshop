...
- **Lab 15 -- Background Agents.** `app.background:` block + Lakebase-backed responses store. Kickoff returns a `resp_*` ID immediately; the client polls `operation: retrieve` (or the deployed app's `/v1/responses/{id}` route) until `completed`. Demonstrates the **async lifecycle** dimension.
...
- **Lab 24 -- UC OTEL Trace Tables.** Set `app.trace_location` on the same supervisor and route traces into three Delta tables (`..._otel_spans`, `..._otel_logs`, `..._otel_metrics`) auto-created in the configured UC schema. Call `mlflow.set_experiment(trace_location=UnityCatalog(...))` from the notebook (post-MLflow-3.11 blessed API), drive traffic, then query the spans table directly via Spark SQL -- the durable foundation for Lakehouse Monitoring dashboards.
...
| 5 | [lab-15-background/](lab-15-background/) | Lab | Responses-API kickoff/poll/cancel + Lakebase-persisted state. |
...
| 14 | [lab-24-uc-trace-location/](lab-24-uc-trace-location/) | Lab | UC OTEL trace tables. `app.trace_location` block + `mlflow.set_experiment(trace_location=UnityCatalog(...))` + Spark SQL query of the three Delta tables (`..._otel_spans` / `..._otel_logs` / `..._otel_metrics`). |
| 15 | [lab-25-agent-as-tool/](lab-25-agent-as-tool/) | Lab | Agent-as-tool with `type: app` + `type: serving_endpoint` (dao-ai 0.1.98). Two deployed apps, one calls the other. |
...
- **Lab 23**: same UC catalog/schema as Lab 22. **SQL warehouse ID is required** -- this lab deploys to Apps and Apps containers can't reach the default MLflow trace export host, so traces would be silently dropped without `app.trace_location` routing them to UC OTEL tables (see Lab 24 for the deeper walkthrough and the README's "Trace persistence on Databricks Apps" section for the post-deploy SP grants).
