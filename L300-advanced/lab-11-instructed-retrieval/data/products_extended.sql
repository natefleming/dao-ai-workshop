...
USING (
  ...
  ) AS v (sku, product_name, category, description, price, in_stock, brand_name, price_tier, sku_prefix, weight_lbs)
) s
ON t.sku = s.sku
WHEN MATCHED THEN UPDATE SET
  ...
  t.price_tier   = s.price_tier,
  t.sku_prefix   = s.sku_prefix,
  t.weight_lbs   = s.weight_lbs
WHEN NOT MATCHED THEN INSERT (
  sku, product_name, category, description, price, in_stock,
  brand_name, price_tier, sku_prefix, weight_lbs
) VALUES (
  s.sku, s.product_name, s.category, s.description, s.price, s.in_stock,
  s.brand_name, s.price_tier, s.sku_prefix, s.weight_lbs
);
